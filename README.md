...
>
> ℹ️ **"라이브 연동"의 범위**: 실데이터 모드는 서울 보행신호의 *실시간 데이터 흐름*을 시연합니다. 다만 경로 신호(s1~s5)에는 실제 교차로 신호를 *순서대로 자동 배정*하며, 경로 위치와 실제 교차로를 정밀 매칭하지는 않습니다(지오코딩·경로탐색은 데모 범위 밖). 즉 표시되는 잔여시간은 진짜지만, 그 신호의 물리적 위치가 화면상 경로와 일치하지는 않습니다.
...
- 프록시는 `signal_api.jsx`의 `PROXIES` **우선순위 목록**으로 관리해 한 곳이 죽어도 다음 프록시로 **자동 failover**(단일 장애점 완화). 본인 Worker를 배포해 목록 맨 앞에 두면 외부 워커 의존을 없앨 수 있음 — 배포법은 [`proxy/cloudflare-worker.js`](proxy/cloudflare-worker.js) 주석 참고.
